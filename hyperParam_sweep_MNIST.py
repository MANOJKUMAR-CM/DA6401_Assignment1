...
def sweep_hyperParameters():
    ...
    # Name
    run_name = f"LR: {learningRate}, AC: {activationFunc}, Optim: {optimizer}"
    print("Sweep Name: ",run_name)
    
    # To Initialize Weights
    layers = [X_train.shape[0]] + [128] * 4 + [Y_train.shape[0]]
    W, b = initialize_weights(layers, "xavier")
    ...
    num_batches = X_train.shape[1] // 64 
    ...
    for epoch in range(20):
        ...
        for i in range(num_batches):
            ...
            if optimizer == "sgd":
                W, b = SGD(W, b, gradients, learningRate, 0)
            elif optimizer == "momentum":
                W, b, uW, ub = Momentum_GD(W, b, uW, ub, gradients, learningRate, beta, 0)
            elif optimizer == "nesterov":
                W, b, uW, ub = Nesterov_Accelerated_GD(W, b, uW, ub, gradients, learningRate, beta, 0)
            elif optimizer == "rmsprop":
                W, b, uW, ub = RMSProp(W, b, uW, ub, gradients, learningRate, beta, eps, 0)
            elif optimizer == "adam":
                W, b, uW, ub = Adam(W, b, uW, ub, mW, mb, gradients, learningRate, eps, i+1, 0)
            elif optimizer == "nadam":
                W, b, uW, ub = NAdam(W, b, uW, ub, mW, mb, gradients, learningRate, eps, i+1, 0)
        ...
        print(f"Epoch {epoch+1}/{20}, Train Loss: {train_loss / num_batches:.4f}, Val Loss: {val_loss}, Val Accuracy: {val_accuracy}")
...
sweep_ID = wandb.sweep(sweep_config, entity="manoj_da24s018-iit-madras", project="DNN-HyperParameter-Sweep-MNIST")
wandb.agent(sweep_ID, sweep_hyperParameters, count=50)
